...
test_loader = DataLoader(InsertDataset(test_config),
...
for bytez, insert_offsets, ori_bytez, bytez_len in test_loader:
  ...
  for i in range(test_config.batch_size):
    if init_result[i]:
      target_idxs.append(torch.Tensor(list(range(insert_offsets[i], insert_offsets[i]+test_config.padding_len))).long())
